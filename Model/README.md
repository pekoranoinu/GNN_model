...
- 假設 `A` 已是 row-normalized adjacency，因此不再額外處理。

### 2. `SAGEBlock`

`SAGEBlock` 的結構為：

`SAGELayer → LayerNorm → PReLU → Dropout → Residual`
...
最終預測時，通常會使用此函式來選擇較保守的決策門檻。流程如下：

- 只在 `[thr_min, thr_max]` 區間內掃描 threshold（預設 0.3～0.7）；
- 對每個候選 threshold 計算：
  - train 上的 recall；
  - validation 上的 recall；
- 若任一側 recall 低於 `min_recall`，即捨棄此 threshold；
- 在「train／validation 超過 `min_recall`」的候選集合中：
